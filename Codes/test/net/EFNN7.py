...
class EFNN7(nn.Module):
    def __init__(self,config,logger_file_name):
        super(EFNN7, self).__init__()
        # self.config = config
        ...
        totallayer = len(self.config.net.deconvlayerunits)
        self.last = nn.ConvTranspose2d(self.config.net.deconvlayerunits[totallayer-2],1,3).to(device)
    ...
    def forward(self,x, mulfactor=9 ):
        ...
        print('...allocation size', allocation.shape)
        return allocation
